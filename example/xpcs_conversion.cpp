...
int main(int argc, char* argv[])
{
    ...
    if (argc < 9) {
        ...
        fprintf(stderr, "   %s -i data/xpcs.bin -d 512 1813 1558 -e 5\n", argv[0]);
        fprintf(stderr, "   %s -i data/xpcs.bin -d 512 1813 1558 -e 3 -o data/xpcs-dec.bin\n", argv[0]);
        return 1;
    }
    ...
    cudaMemset(d_cmpBytes, 0, nbEle*sizeof(float)); // set to zero for double check.
    ...
    if(strlen(decFilePath) > 0) {
        ...
        for (size_t i = 0; i < nbEle; i++) {
            decData[i] = (uint16_t)dup_decData[i];
        }
