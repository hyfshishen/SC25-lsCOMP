...
int main(int argc, char* argv[])
{
    ...
    if (argc < 9) {
        ...
        fprintf(stderr, "   %s -i data/xpcs.bin -d 600 1813 1558 -t 5\n", argv[0]);
        fprintf(stderr, "   %s -i data/xpcs.bin -d 600 1813 1558 -t 3 -o data/xpcs-dec.bin\n", argv[0]);
        return 1;
    }
    ...
    for(int i=0; i<3; i++) cuSZp_compress_plain_f32_truncation_xpcs(d_oriData, d_cmpBytes, nbEle, &cmpSize, (uint)threshold, stream);
    printf("GPU warmup finished!\n\n");

    // cuSZp compression.
    timer_GPU.StartCounter();
    cuSZp_compress_plain_f32_truncation_xpcs(d_oriData, d_cmpBytes, nbEle, &cmpSize, (uint)threshold, stream);
    ...
    // cuSZp decompression.
    timer_GPU.StartCounter();
    cuSZp_decompress_plain_f32_truncation_xpcs(d_decData, d_cmpBytes, nbEle, cmpSize, (uint)threshold, stream);
