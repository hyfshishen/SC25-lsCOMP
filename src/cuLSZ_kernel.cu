...
__global__ void cuSZp_compress_kernel_plain_f32_truncation_cssi(const uint32_t* const __restrict__ oriData, 
                                                                unsigned char* const __restrict__ cmpData, 
                                                                volatile unsigned int* const __restrict__ cmpOffset, 
                                                                volatile unsigned int* const __restrict__ locOffset, 
                                                                volatile int* const __restrict__ flag, 
                                                                const uint threshold, const size_t nbEle)
{
    __shared__ unsigned int excl_sum;
    __shared__ unsigned int base_idx;
    ...
    for(int j=0; j<block_num; j++)
    {
        ...
        if(base_block_end_idx < nbEle)
        {
            ...
            for(int i=base_block_start_idx; i<base_block_end_idx; i+=4)
            {
                tmp_buffer = reinterpret_cast<const uint4*>(oriData)[i/4];
                quant_chunk_idx = j * 32 + i % 32;

                currQuant = tmp_buffer.x > threshold ? tmp_buffer.x : 0;
                lorenQuant = currQuant - prevQuant;
                prevQuant = currQuant;
                sign_ofs = i % 32;
                sign_flag[j] |= (lorenQuant < 0) << (31 - sign_ofs);
                ...
                currQuant = tmp_buffer.y > threshold ? tmp_buffer.y : 0;
                lorenQuant = currQuant - prevQuant;
                prevQuant = currQuant;
                sign_ofs = (i+1) % 32;
                sign_flag[j] |= (lorenQuant < 0) << (31 - sign_ofs);
                ...
                currQuant = tmp_buffer.z > threshold ? tmp_buffer.z : 0;
                lorenQuant = currQuant - prevQuant;
                prevQuant = currQuant;
                sign_ofs = (i+2) % 32;
                sign_flag[j] |= (lorenQuant < 0) << (31 - sign_ofs);
                ...
                currQuant = tmp_buffer.w > threshold ? tmp_buffer.w : 0;
                lorenQuant = currQuant - prevQuant;
                prevQuant = currQuant;
                sign_ofs = (i+3) % 32;
                sign_flag[j] |= (lorenQuant < 0) << (31 - sign_ofs);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    quant_chunk_idx = j * 32 + i % 32;
                    currQuant = oriData[i] > threshold ? oriData[i] : 0;
                    lorenQuant = currQuant - prevQuant;
                    prevQuant = currQuant;
                    sign_ofs = i % 32;
                    sign_flag[j] |= (lorenQuant < 0) << (31 - sign_ofs);
...
__global__ void cuSZp_decompress_kernel_plain_f32_truncation_cssi(uint32_t* const __restrict__ decData, 
                                                                const unsigned char* const __restrict__ cmpData, 
                                                                volatile unsigned int* const __restrict__ cmpOffset, 
                                                                volatile unsigned int* const __restrict__ locOffset, 
                                                                volatile int* const __restrict__ flag, 
        ...
        if(fixed_rate[j])
        {
            ...
            if(base_block_end_idx < nbEle)
            {
                ...
                for(int i=0; i<32; i+=4)
                {
                    ...
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.x = (uint)currQuant;

                    sign_ofs = (i+1) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+1] * -1 : absQuant[i+1];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.y = (uint)currQuant;

                    sign_ofs = (i+2) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+2] * -1 : absQuant[i+2];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.z = (uint)currQuant;

                    sign_ofs = (i+3) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+3] * -1 : absQuant[i+3];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.w = (uint)currQuant;
                    
                    reinterpret_cast<uint4*>(decData)[(base_block_start_idx+i)/4] = dec_buffer;
                }
            }
            else
            {
                for(int i=0; i<32; i++)
                {
                    ...
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    if(base_block_start_idx+i < nbEle) decData[base_block_start_idx+i] = (uint)currQuant;
                }
            }      
        }
...
__global__ void cuSZp_compress_kernel_plain_f32_truncation_xpcs(const uint16_t* const __restrict__ oriData, 
                                                                unsigned char* const __restrict__ cmpData, 
                                                                volatile unsigned int* const __restrict__ cmpOffset, 
                                                                volatile unsigned int* const __restrict__ locOffset, 
                                                                volatile int* const __restrict__ flag, 
                                                                const uint threshold, const size_t nbEle)
{
    ...
    for(int j=0; j<block_num; j++)
    {
        ...
        if(base_block_end_idx < nbEle)
        {
            ...
            for(int i=base_block_start_idx; i<base_block_end_idx; i+=4)
            {
                tmp_buffer = reinterpret_cast<const ushort4*>(oriData)[i/4];
                quant_chunk_idx = j * 32 + i % 32;
...
__global__ void cuSZp_decompress_kernel_plain_f32_truncation_xpcs(uint16_t* const __restrict__ decData, 
                                                                const unsigned char* const __restrict__ cmpData, 
                                                                volatile unsigned int* const __restrict__ cmpOffset, 
                                                                volatile unsigned int* const __restrict__ locOffset, 
                                                                volatile int* const __restrict__ flag, 
        ...
        if(fixed_rate[j])
        {
            ...
            if(base_block_end_idx < nbEle)
            {
                ...
                for(int i=0; i<32; i+=4)
                {
                    ...
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.x = (ushort)currQuant;

                    sign_ofs = (i+1) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+1] * -1 : absQuant[i+1];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.y = (ushort)currQuant;

                    sign_ofs = (i+2) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+2] * -1 : absQuant[i+2];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.z = (ushort)currQuant;

                    sign_ofs = (i+3) % 32;
                    lorenQuant = sign_flag & (1 << (31 - sign_ofs)) ? absQuant[i+3] * -1 : absQuant[i+3];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.w = (ushort)currQuant;
                    
                    reinterpret_cast<ushort4*>(decData)[(base_block_start_idx+i)/4] = dec_buffer;
                }
            }
            else
            {
                for(int i=0; i<32; i++)
                {
                    ...
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    if(base_block_start_idx+i < nbEle) decData[base_block_start_idx+i] = (ushort)currQuant;
                }
            }      
        }
